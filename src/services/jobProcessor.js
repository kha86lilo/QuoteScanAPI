...
class JobProcessor {
  ...
  async cleanupOldJobs(daysOld = 7) {
    ...
    if (removedFromMemory > 0) {
      console.log(`Cleanup: Removed ${removedFromMemory} old jobs from memory cache (database records preserved)`);
    }
  }
    ...
    try {
      await client.query(`
        INSERT INTO processing_jobs (
          ...
          completed_at, job_data, result, error, progress, summary
    ...
    try {
      await client.query(`
        ...
        SET status = $2, updated_at = $3, started_at = $4, 
            completed_at = $5, result = $6, error = $7, progress = $8, summary = $9
        WHERE job_id = $1
      `, [
        job.id,
        job.status,
